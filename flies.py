...
def init_random(myclass,n,vel_scale=2,WORLD_SIZE=20,DELTA_T=1):
    xs = WORLD_SIZE*np.random.random(n)-WORLD_SIZE/2
    ys = WORLD_SIZE*np.random.random(n)-WORLD_SIZE/2
...
class Fly():
    ...
    @staticmethod
    def wrap_position(position, WORLD_SIZE):
        position[position > WORLD_SIZE] -= 2*WORLD_SIZE
        position[position < -WORLD_SIZE] += 2*WORLD_SIZE
        return position
    ...
    def get_distance(self,target):
        '''for single target'''
        return np.linalg.norm(self.wrap_position(target.position - self.position, self.WORLD_SIZE))
    ...
    def seek(self,target):
        target_direction = self.angle_to_heading(self.heading_to_angle(self.wrap_position(target.position - self.position, self.WORLD_SIZE)))
    ...
    def collisions(self,targets=[]):
        ...
        for target in targets:
            ...
            if distance < self.collision_radius:
                target_relative_position = self.wrap_position(target.position - self.position, self.WORLD_SIZE)
